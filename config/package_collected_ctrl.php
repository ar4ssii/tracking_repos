...
if (isset($_POST['btn_confirm_drop_off'])) {
    ...
    $EstimatedDeliveryDate = date('Y-m-d H:i:s', strtotime($DateDroppedOff . ' +3 days'));
    ...
    if ($result_CheckDeliverStatus->num_rows > 0) {
        ...
        if ($result_UpdateSender && $result_UpdateRecipient && $result_UpdateDeliverStatus) {
            echo 'update success';
            header('location: ../admin-files/managePackages.php');
            $_SESSION['message'] = 'Confirmation: Package set status to collected succeeded';
            $_SESSION['alert-color'] = 'success';
            header('location: ../admin-files/forApproval.php');
        } else {
            echo 'error, ewan bakit';
            $_SESSION['message'] = 'Error: Package set status to collected failed';
            $_SESSION['alert-color'] = 'danger';
            header('location: ../admin-files/forApproval.php');
        }
    } else {
        echo 'walang nakitang deliverID katulad ng sinubmit mo';
